...
export function chartFactory(opts, proto = protoChart) {
  ...
  if (chart.key_panel) {
    if (chart.chartType === 'line') {
      chart.container_width *= 0.9;
    } else {
      chart.container_height -= chart.container_height * 0.2;
    }
  }
  ...
  if (chart.assignment_ribbon) {
    chart.container_height -= chart.container_height * 0.1;
  }
  ...
  chart.buttons = chart.svg.append('g')
    .attr('id', 'buttons')
    .attr("width", (chart.width * 0.5) - chart.margin.left - chart.margin.right)
    .attr("height", chart.height)
...
export function button_helper(chart, label, type, xoffset, layer = "buttons", x_addition = 0, y_addition = 0) {
    const x_pos = xoffset + x_addition;
    const y_pos = y_addition;
    
    // Adjust button size as needed
    const buttonWidth = chart.em_size * 8; // Adjusted width
    const buttonHeight = chart.em_size * 1.5; // Adjusted height
  
    const buttonId = `${label}${type}Button`;
    ...
    const handleMouseOut = () => {
      d3.select(`#${buttonId}`).classed('active', false);
    };
  
    // Append the button rectangle with Bootstrap classes
    chart[layer].append("foreignObject")
      .attr('x', x_pos)
      .attr('y', y_pos)
      ...
      .html(`<span class="text-white">Save ${type.toUpperCase()}</span>`)
